...
interface DataInterface {
  ...
  market_store_count: number | null
  market_store_percentage: number | null
...
interface BodyInterface {
  ...
  market_store_count: number | null
  market_store_percentage: number | null
  market_store_percentage_txt: string | null
  sun: number | null
...
const header: ColumnInterface[] = [
  ...
  },
  {
    ...
    key: 'market_store_percentage_txt',
    textAlign: 'center',
    width: 2,
    sortKey: 'market_store_percentage',
  },
  {
    content: 'Qtd. PDVs',
    key: 'market_store_count',
    textAlign: 'center',
    width: 2,
    sortKey: 'market_store_count',
  },
  {
    content: 'D',
    key: 'sun_jsx',
...
const parseData = (data: DataInterface[]): BodyInterface[] => {
  ...
  return data.map((e) => {
    const data = {
      ...
      market_name: notEmptyString(e.market_name) ? e.market_name : null,
      market_store_count: isNumeric(e.market_store_count)
        ? e.market_store_count
        : null,
      market_store_percentage: null,
      market_store_percentage_txt: null,
    ...
    if (isNumeric(e.market_store_percentage)) {
      const value = e.market_store_percentage
      data.market_store_percentage = value
      data.market_store_percentage_txt = `${value}%`
    }
...
const request = async (
  ...
  const { data } = await axios.get('/v1/widgets/details/28', { params })
...
const Manager = (props: ManagerProps) => {
  ...
  const extractData = async (): Promise<any> => {
    ...
    try {
      const {
        data: { data, success },
      } = await axios.get('/v1/widgets/details/28.xlsx')
