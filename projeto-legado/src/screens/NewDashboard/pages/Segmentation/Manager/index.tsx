...
interface BodyInterface {
  ...
  segment_store_count: number | null
  segment_store_percentage: number | null
  segment_store_percentage_txt: string | null
  sun: number | null
...
const header: ColumnInterface[] = [
  ...
  },
  {
    ...
    key: 'segment_store_percentage_txt',
    textAlign: 'center',
    width: 2,
  ...
  return data.map((e) => {
    const data = {
      ...
      segment_store_count: isNumeric(e.segment_store_count)
        ...
        : null,
      segment_store_percentage: null,
      segment_store_percentage_txt: null,
    ...
    if (isNumeric(e.segment_store_percentage)) {
      const value = e.segment_store_percentage
      data.segment_store_percentage = value
      data.segment_store_percentage_txt = `${value}%`
    }
...
const request = async (
  ...
  const { data } = await axios.get('/v1/widgets/details/25', { params })
...
const Manager = (props: ManagerProps) => {
  ...
  const extractData = async (): Promise<any> => {
    ...
    try {
      const {
        data: { data, success },
      } = await axios.get('/v1/widgets/details/25.xlsx')
