...
export function Widget36({ setOpen, onHandleOptions, segment_id }) {
  const dispatch = useDispatch()
  const { widget36 } = useSelector((state: any) => state.dashboard)
  const { loading, data } = widget36

  const loadWidget36 = () => {
    dispatch(actions.getWidget36({ date: currentDate, segment_id }))
  }
  ...
  return (
    <>
      {loading ? (
        ...
        <Container className='avg-speed'>
          <header>
            <strong> Velocidade Média vs Média Móvel 21 dias </strong>
            <AiOutlineExpandAlt
