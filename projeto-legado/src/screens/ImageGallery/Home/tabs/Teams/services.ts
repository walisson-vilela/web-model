...
export const listTeams = async (
  ...
  const response = await listTeamsRequest(queryParams)
...
// Essa função irá fazer a requisição para extração dos dados
export const extractTeams = async (queryParams: QueryParams): Promise<void> => {
  const { data } = await listTeamsRequest(queryParams, true)
