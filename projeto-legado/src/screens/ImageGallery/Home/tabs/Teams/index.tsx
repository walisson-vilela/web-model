...
import { extractTeams, listTeams } from './services'
...
const Manager: TabComponent = (props) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const responseData = await listTeams({
        appliedFilters,
        search,
        sort,
        dateInterval,
  ...
  const noneSelectedRule = {
    rule: () => {
      return (
        checkeds.filter((checked) => isNumber(checked.people_id)).length > 0
      )
    },
    message:
      'Para realizar a ação é necessário selecionar pelo menos uma Equipe',
  }
  ...
  const onClickExtractData = async (): Promise<any> => {
    ...
    const ids = checkeds.map((e) => e.people_id)
  ...
  useEffect(() => {
    checkeds.map((item) => {
      if (!tourIds.includes(item.people_id)) {
        setTourIds((prev) => [...prev, item.people_id])
      }
    })
  }, [checkeds])
          ...
          trigger={
            <div>
              <MwButton
                ...
                style={{ width: '110px' }}
                onClick={() => {
                  const pathname = `gallery/${checkeds.map((e) => e.people_id)}`
                  ...
                  const params = {
                    ...
                    appliedFilters:
                      appliedFilters.length > 0
                        ? appliedFilters
                            .filter((item) => item.name !== 'people_id')
                            .map((props) => props)
                        : [],
                  }
                  history.push({
