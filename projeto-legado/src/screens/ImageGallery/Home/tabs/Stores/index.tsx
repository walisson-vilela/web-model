...
import { extractStores, listStores } from './services'
...
const Manager: TabComponent = (props) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const responseData = await listStores({
        appliedFilters,
        search,
        sort,
        dateInterval,
  ...
  const noneSelectedRule = {
    rule: () => {
      return checkeds.filter((checked) => isNumber(checked.store_id)).length > 0
    },
  ...
  const onClickExtractData = async (): Promise<any> => {
    ...
    const ids = checkeds.map((e) => e.store_id)
  ...
  useEffect(() => {
    checkeds.map((item) => {
      if (!tourIds.includes(item.store_id)) {
        setTourIds((prev) => [...prev, item.store_id])
      }
    })
  }, [checkeds])
        ...
        <Popup
          ...
          content={
            <div style={{ width: '264px' }}>
              <span>
                Para realizar à ação é necessário selecionar pelo menos um PDV.
              </span>
            </div>
          }
          trigger={
            <div>
              <MwButton
                ...
                style={{ width: '110px' }}
                onClick={() => {
                  const pathname = `gallery/${checkeds.map((e) => e.store_id)}`
                  ...
                  const params = {
                    ...
                    appliedFilters:
                      appliedFilters.length > 0
                        ? appliedFilters
                            .filter((item) => item.name !== 'store_id')
                            .map((props) => props)
                        : [],
                  }
                  history.push({
