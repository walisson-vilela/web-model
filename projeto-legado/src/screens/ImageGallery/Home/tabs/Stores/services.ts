...
export const listStores = async (
  ...
  const response = await listStoresRequest(queryParams)
...
// Essa função irá fazer a requisição para extração dos dados
export const extractStores = async (
  queryParams: QueryParams,
): Promise<void> => {
  const { data } = await listStoresRequest(queryParams, true)
