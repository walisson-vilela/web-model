...
import { extractdForms, listdForms } from './services'
...
const Manager: TabComponent = (props) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const responseData = await listdForms({
        appliedFilters,
        search,
        sort,
        dateInterval,
  ...
  const noneSelectedRule = {
    rule: () => {
      return checkeds.filter((checked) => isNumber(checked.form_id)).length > 0
    },
    message:
      'Para realizar a ação é necessário selecionar pelo menos um Formulário',
  }
  ...
  const onClickExtractData = async (): Promise<any> => {
    ...
    const ids = checkeds.map((e) => e.form_id)
  ...
  useEffect(() => {
    checkeds.map((item) => {
      if (!tourIds.includes(item.form_id)) {
        setTourIds((prev) => [...prev, item.form_id])
      }
    })
  }, [checkeds])
        ...
        <Popup
          ...
          content={
            <div style={{ width: '264px' }}>
              <span>
                Para realizar à ação é necessário selecionar pelo menos um
                Formulário.
              </span>
            </div>
          }
          trigger={
            <div>
              <MwButton
                ...
                style={{ width: '110px' }}
                onClick={() => {
                  const pathname = `gallery/${checkeds.map((e) => e.form_id)}`
