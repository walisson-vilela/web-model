...
const getData = (week, params) =>
  ((dispatch) => {
    ...
    dispatch(
      getWidgetData(31, 'POST', '/v1/widgets/stats/31', data, {
        segment_id: params,
      }),
    )
  }) as never
...
export const Widget31 = ({ segment_id }) => {
  const dispatch = useDispatch()
  ...
  const loading = useSelector((state: any) => state.w31.loading)
  const data = useSelector((state: any) => state.w31.data)
  ...
  return (
    <>
      <Columns
        header={{
          text: 'Tempo de Atendimento (Desvio Padrão 2.0)',
          expand: true,
        }}
        options={options}
        loading={loading}
