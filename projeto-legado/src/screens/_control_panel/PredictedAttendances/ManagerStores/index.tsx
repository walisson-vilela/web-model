...
const Manager = (props: ManagerProps) => {
  ...
  // estado controlador do filtro de data
  ...
  }, [appliedFilters, search, sort, page, date])
  ...
  useEffect(() => {
    setBody(parseData(data, setModal, setOpenModal))
  }, [data])
  ...
  const onClickRefuseAttendance = (item: BodyInterface) => {
    // setModal(RefuseAttendance({
    //   close: () => setModal(null),
    // }))
    setModal(
      <RefuseAttendance
  ...
  const getItemMenu = (item: BodyInterface): DropdownInterfaces.Item[] => {
    return [
      {
        content: 'Recusar Atendimento',
        onClick: () => onClickRefuseAttendance(item),
        rules: [],
      },
    ]
  }
