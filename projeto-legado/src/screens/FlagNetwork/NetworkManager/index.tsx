...
const Manager = (props: ManagerProps) => {
  ...
  const noneSelectedRule = {
    ...
    message: 'Para realizar a ação é necessário selecionar pelo menos uma rede',
  }

  const onClickActivate = () => {
    const toUpdate = checkeds.filter(
      (checked) => !checked.active && checked.flag_count_value > 0,
    )
    ...
    setConfirmModal({
      title: `Ativar Rede${toUpdate.length > 1 ? 's' : ''}`,
      content:
        toUpdate.length === 1 ? (
          inactives.length > toUpdate.length ? (
            <React.Fragment>
              Para ativar a Rede é necessário definir a visão da Bandeira.
              <br />
              Deseja ativar a Rede <b>{toUpdate[0].name.toUpperCase()}</b>?
              <br />
              Esta ação irá ativar o Grupo e todas as Bandeiras da hierarquia
            </React.Fragment>
          ) : (
            <React.Fragment>
              Deseja ativar a Rede <b>{toUpdate[0].name.toUpperCase()}</b>?
          ...
          <React.Fragment>
            Para ativar a Rede é necessário definir a visão da Bandeira.
            <br />
            Deseja ativar as <b>{toUpdate.length} Redes</b> selecionadas?
            <br />
            Esta ação irá ativar os Grupos e todas as Bandeiras das hierarquias
          </React.Fragment>
        ) : (
          <React.Fragment>
            Deseja ativar as <b>{toUpdate.length} Redes</b> selecionadas?
  ...
  const onClickInactivate = () => {
    ...
    setConfirmModal({
      title: `Inativar Rede${toUpdate.length > 1 ? 's' : ''}`,
      content:
        toUpdate.length === 1 ? (
          <React.Fragment>
            Você deseja inativar a Rede <b>{toUpdate[0].name}</b>?<br />A ação
            inativa todas as Bandeiras abaixo e sendo a Rede a única ativa, irá
            inativar junto o Grupo.
          </React.Fragment>
        ) : (
          <React.Fragment>
            Você deseja inativar as <b>{toUpdate.length} Redes</b> selecionadas?
            <br />A ação inativa todas as Bandeiras abaixo e sendo a Rede a
            única ativa, irá inativar junto o Grupo.
          </React.Fragment>
        ),
      actions: [
        {
  ...
  const onClickDelete = () => {
    ...
    setConfirmModal({
      title: `Deletar Rede${toUpdate.length > 1 ? 's' : ''}`,
      content:
        toUpdate.length === 1 ? (
          <React.Fragment>
            Você deseja deletar a Rede <b>{toUpdate[0].name}</b>? <br /> A ação
            deleta todas as Bandeiras abaixo e sendo a Rede a única ativa, irá
            inativar junto o Grupo.
          </React.Fragment>
        ) : (
          <React.Fragment>
            Você deseja deletar as <b>{toUpdate.length} Redes</b> selecionadas?
            A ação deleta todas as Bandeiras abaixo e sendo a Rede a única
            ativa, irá inativar junto o Grupo
          </React.Fragment>
        ),
      actions: [
        {
  ...
  const dropdownItems = [
    {
      ...
      rules: [
        noneSelectedRule,
        {
          ...
          message:
            'Para realizar a ação é necessário selecionar pelo menos uma rede inativa.',
        },
        {
          rule: () => {
            ...
            return invalid === 0 || (invalid > 0 && invalid < checkeds.length)
          },
          message: 'Para ativar a Rede verifique se a Rede possui Bandeira.',
        },
      ],
    },
    {
      ...
      rules: [
        noneSelectedRule,
        {
          rule: () => checkeds.filter((checked) => checked.active).length > 0,
          message:
            'Para realizar a ação é necessário selecionar pelo menos uma rede ativa',
        },
      ],
    },
    {
