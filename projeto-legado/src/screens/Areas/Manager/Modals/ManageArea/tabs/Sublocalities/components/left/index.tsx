...
const useLeft: TUseContentSelected<Sublocality> = ({
  ...
  const { loading: loadingContext } = useManageAreaContext()
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const rows = response.map((data): Rows<Sublocality>[number] => {
    ...
    setLoading(false)
  }, [search, appliedFilters, selected, loadingContext, page])
      ...
      messages: {
        empty:
          search.length > 0 || appliedFilters.length > 0
            ...
            : 'Utilize o campo de busca acima para pesquisar por Bairros',
      },
      toolbar: {
        checkAll: true,
        search: {
