    ...
    setModal(
      <DefineGoals
        ...
        messages={{
          title:
            checkeds.length === 1
              ? 'Foi selecionado 1 PDV'
              : `Foram selecionados ${checkeds.length} PDVs`,
          label: 'Atribua uma meta para o tempo de atendimento dos PDVs',
        }}
      />,
    )
  }
  ...
  const onClickClearGoals = () => {
    setModal({
      ...
      content:
        checkeds.length === 1 ? (
          <React.Fragment>
            Você deseja limpar as metas do PDV selecionado?
          </React.Fragment>
        ) : (
          <React.Fragment>
            Você deseja limpar as metas dos {checkeds.length} PDVs selecionados?
          </React.Fragment>
        ),
      actions: [
        {
      ...
      content:
        checkeds.length === 1 ? (
          <React.Fragment>
            ...
            <b>{checkeds.length} PDV</b> selecionado?
          </React.Fragment>
        ) : (
          <React.Fragment>
            Você deseja atribuir como meta o tempo médio respectivo <br />
            as <b>{checkeds.length} PDVs</b> selecionados?
          </React.Fragment>
        ),
      actions: [
        {
