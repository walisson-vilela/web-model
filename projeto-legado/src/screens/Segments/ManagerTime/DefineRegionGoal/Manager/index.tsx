...
const Manager = (props: ManagerProps) => {
  ...
  useEffect(() => {
    setParsed(parseData(data, segment.id, setModal, setDurationGoals))
  }, [data])
    ...
    setModal(
      <DefineGoals
        ...
        messages={{
          title:
            checkeds.length === 1
              ? 'Foi selecionada 1 área'
              : `Foram selecionadas ${checkeds.length} áreas`,
          label: 'Atribua uma meta para o tempo de atendimento das áreas',
        }}
      />,
    )
  }
  ...
  const onClickClearGoals = () => {
    setModal({
      ...
      content:
        checkeds.length === 1 ? (
          <React.Fragment>
            Você deseja limpar as metas da área selecionada?
          </React.Fragment>
        ) : (
          <React.Fragment>
            Você deseja limpar as metas das {checkeds.length} áreas
            selecionadas?
          </React.Fragment>
        ),
      actions: [
      ...
      content:
        checkeds.length === 1 ? (
          <React.Fragment>
            ...
            <b>{checkeds.length} Área</b> selecionada?
          </React.Fragment>
        ) : (
          <React.Fragment>
            Você deseja atribuir como meta o tempo médio respectivo <br />
            as <b>{checkeds.length} Áreas</b> selecionadas?
          </React.Fragment>
        ),
      actions: [
        {
